...
정의: 문자열 $S$에서 $|S|$는 $S$의 길이이다.

정의: 문자열 $S$에서 $S[i]$는 $S$의 $i$번째 문자이다. 여기서 인덱스는 $0$-based indexing을 따른다.

정의: 문자열 $S$에서 $S[i:j]$는 $S[i], S[i+1], S[i+2], \dots, S[j-1]$를 순서대로 이은 문자열이다. $i = 0$이거나 $j = |S|$인 경우에는 각각 $i$ 또는 $j$를 생략할 수 있다.

정의: 두 문자열 $S, T$에 대해 $S + T$는 $S$와 $T$를 순서대로 이어 붙인 문자열이다.

정의: Circular LCS는 두 문자열 $S, T$에 대해 모든 $0 \le i < |S|$에 대하여 $S[i:] + S[:i]$와 $T$의 LCS 길이를 구하는 문제이다.
...
정의: 문자열 $S$의 접두사는 어떤 $0 \le i \le |S|$에 대해 $S[:i]$인 문자열이다.

정의: 문자열 $S$의 접미사는 어떤 $0 \le i \le |S|$에 대해 $S[i:]$인 문자열이다.
