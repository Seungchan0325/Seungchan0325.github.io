...
입니다. $P_e - P_{s-1}$를 최대화 하기 위해선 $P_e$가 가장 큰 값, $P_{s-1}$가 가장 작은 값 이여야합니다. 이를 식으로 표현하면 이와 같습니다.

$\max_{k = x2}^{y2}P_k - \min_{k = x1-1}^{y1-1}P_k$

s값은 -1한 상태의 min값을 구해야하기 때문에 $[x1-1, y1-1]$구간에서 min값을 구해야합니다. 그래서 세그먼트 트리로 min값과 max값을 관리하면 $(x1, y1)$와 $(x2, y2)$가 겹치지 않은 경우를 계산할 수 있습니다. 
...
3. $x2 \leq s \leq e \leq y1$ 인 경우 세그먼트 트리에서 두 자식 노드를 합칠때 (merge 함수) 왼쪽 자식의 min값과 오른쪽 자식의 max값의 차이, 왼쪽 자식의 max diff와 오른쪽 자식의 max diff 중 가장 큰 값을 선택합니다.
