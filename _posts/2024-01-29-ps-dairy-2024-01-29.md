...
### 관측 2
> 왼쪽에서 오른쪽으로 방문 할 때 $i (1 \leq i \leq N)$에 마지막으로 방문한 경우 최댓값은 이전 D개의 최댓값 + 수열의 $i$ 번째 항입니다.

이제 [관측 1](#관측-1)을 바탕으로 답을 구해야 합니다. 왼쪽에서 오른쪽 방향으로 방문한다고 할 때 $i (1 \leq i \leq N)$을 마지막으로 방문하였을 때 방문한 수열들의 합의 최댓값은 무엇일까요?

$i$항 이전에 방문한 항 $j (1 \leq j \leq N)$는 $i - d \leq j \leq i$임이 자명합니다. 따라서 $f(i) = i$를 마지막으로 방문하였을 때 방문한 수열들의 합의 최댓값이라면 정의하면 $f(i) = S_i + \max_{k = i - D}^{k \lt i} f(k)$입니다.
...
For (i = 1; i <= N; i++) {
    ...
    DP[i] = get_max_element(DP, i - 1) + S[i];
}
